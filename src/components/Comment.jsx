...
function Comment(props) {
  ...
  const [highlightedScore, setHighlightedScore] = useState(null);

  const handleScoreHighlight = (score) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (!auth) {
      ...
      } else if (!commentRef.current.value || !selectedScore) {
        ...
        if (
          ...
          setErrorMessage(""); // Reset error message on successful submission
        }
            ...
            {scores.map((score, index) => (
              <React.Fragment key={score}>
                <span
                  ...
                  onMouseEnter={() => handleScoreHighlight(score)}
                  onMouseLeave={() => handleScoreHighlight(null)}
                  onClick={() => handleScoreSelection(score)}
                >
                  <FaStar
                    style={{
                      color:
                        (selectedScore || highlightedScore) >= score
                          ? "#FFAE42"
                          : "grey",
                    }}
                  />
                ...
                <div className="commentContent d-flex-column align-items-center ms-3 mb-3">
                  ...
                  <p className="score m-0">
                    {Array.from({ length: comment.score }, (_, index) => (
                      <FaStar style={{ color: "#FFAE42", marginRight: "5px", fontSize: "24px"}} key={index} />
                    ))}
                  </p>
